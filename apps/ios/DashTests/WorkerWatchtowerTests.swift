...
@Test func watchtowerFanoutStillUsesOnlyTheFirstTenZones() throws {
  let zones = try makeZones(count: 15)
  #expect(WatchtowerEngine.zonesForFanout(zones).map(\.id) == Array(zones.prefix(10)).map(\.id))
}

@Test func watchtowerZoneChecksShareOneGlobalWindowOfFour() async throws {
  let zones = try makeZones(count: 10)
  let probe = ConcurrencyProbe(delay: .milliseconds(30))

  _ = try await WatchtowerEngine.loadZoneChecks(
    zones: zones,
    certificateLoader: { _ in
      try await probe.run([CertificatePack]())
    },
    healthcheckLoader: { _ in
      try await probe.run([Healthcheck]())
    })

  let maximumConcurrency = await probe.maximum
  let invocations = await probe.invocations
  #expect(maximumConcurrency == WatchtowerEngine.zoneScopedConcurrency)
  #expect(invocations == zones.count * 2)
}

@Test func watchtowerZoneChecksReportPartialCoverage() async throws {
  let zones = try makeZones(count: 3)
  let result = try await WatchtowerEngine.loadZoneChecks(
    zones: zones,
    certificateLoader: { zone in
      if zone.id == "zone-0" { throw WorkerDetailStubError.failedRoute }
      return []
    },
    healthcheckLoader: { zone in
      if zone.id == "zone-1" {
  ...
  #expect(result.certificates.attemptedCount == 3)
  #expect(result.certificates.failedCount == 1)
  #expect(result.certificates.checkedCount == 2)
  #expect(result.certificates.permissionDeniedCount == 0)
  #expect(result.healthchecks.attemptedCount == 3)
  #expect(result.healthchecks.failedCount == 1)
  #expect(result.healthchecks.checkedCount == 2)
  #expect(result.healthchecks.permissionDeniedCount == 1)

  let signal = WatchtowerEngine.coverageSignal(
    totalZones: 3,
    certificateChecks: result.certificates,
    healthcheckChecks: result.healthchecks)
  #expect(signal?.id == WatchtowerEngine.coverageSignalID)
  #expect(signal?.detail.contains("2 of 3") == true)
}

@Test func watchtowerZoneChecksPropagateCancellation() async throws {
  let zones = try makeZones(count: 10)
  let probe = ConcurrencyProbe(delay: .seconds(30))
  let task = Task {
    try await WatchtowerEngine.loadZoneChecks(
      zones: zones,
      certificateLoader: { _ in
        try await probe.run([CertificatePack]())
      },
  ...
  do {
    _ = try await task.value
    Issue.record("Expected cancellation to leave the Watchtower zone-check scheduler")
  } catch is CancellationError {
    // Expected.
  }
}
