...
// MARK: - Local expiry reminders

@Test func expiryRemindersScheduleOneNoticePerLeadTime() {
  var calendar = Calendar(identifier: .gregorian)
  calendar.timeZone = TimeZone(identifier: "UTC")!
  let now = calendar.date(from: DateComponents(year: 2026, month: 1, day: 1))!
  let expires = calendar.date(from: DateComponents(year: 2026, month: 6, day: 1))!

  let plans = ExpiryReminders.plans(
    subject: .domain,
  ...
  #expect(plans.count == ExpiryReminders.leadDays.count)
  #expect(Set(plans.map(\.identifier)).count == plans.count)
  #expect(plans.allSatisfy { $0.fireDate.hour == ExpiryReminders.fireHour })
  #expect(plans.allSatisfy { $0.body.contains("example.com") })
  // 30 days before June 1 is May 2.
  #expect(plans[0].fireDate.month == 5)
  #expect(plans[0].fireDate.day == 2)
}

@Test func expiryRemindersSkipLeadTimesAlreadyPast() {
  var calendar = Calendar(identifier: .gregorian)
  calendar.timeZone = TimeZone(identifier: "UTC")!
  // Three days out: the 30- and 7-day notices are already behind us, and
  // firing them now would announce a deadline as if it were fresh.
  let now = calendar.date(from: DateComponents(year: 2026, month: 6, day: 1, hour: 12))!
  let expires = calendar.date(from: DateComponents(year: 2026, month: 6, day: 4))!
...
@Test func expiredDomainsScheduleNothing() {
  var calendar = Calendar(identifier: .gregorian)
  calendar.timeZone = TimeZone(identifier: "UTC")!
  let now = calendar.date(from: DateComponents(year: 2026, month: 7, day: 1))!
  let expires = calendar.date(from: DateComponents(year: 2026, month: 6, day: 1))!
  #expect(
...
@Test func reminderIdentifiersAreAccountScoped() {
  // Two accounts can hold the same domain; their reminders must not collide.
  let a = ExpiryReminders.identifier(
    subject: .domain, accountID: "acc-1", resourceID: "z1", leadDays: 7)
...
@Test func expiryDatesParseWithAndWithoutFractionalSeconds() {
  #expect(ExpiryReminders.date(fromISO8601: "2026-06-01T00:00:00Z") != nil)
  #expect(ExpiryReminders.date(fromISO8601: "2026-06-01T00:00:00.000Z") != nil)
  #expect(ExpiryReminders.date(fromISO8601: "2026-06-01") != nil)
  #expect(ExpiryReminders.date(fromISO8601: "not a date") == nil)
}
