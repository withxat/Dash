...
- `Localizable.xcstrings` is a Resources member of **all three** bundles (Dash, DashWidgets, DashShare). It used to be app-only, so the widget and Live Activity resolved their `Text` literals against a bundle with no catalog and shipped English no matter the language setting.
