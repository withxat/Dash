  ...
  var body: some View {
    ZStack {
      ...
      ForEach(0..<layout.outerPetalCount, id: \.self) { index in
        ...
        let expandDelay =
          Double(index + layout.innerPetalCount) * BlossomConstants.petalAnimationDelay
        ...
        if index == 0 {
          ...
          .animation(
            .blossom(delay: model.isExpanded ? expandDelay : collapseDelay), value: model.isExpanded
          )
          .position(center)
        } else {
          PetalView(
      ...
      ForEach(0..<layout.innerPetalCount, id: \.self) { index in
        ...
        let collapseDelay =
          Double(index + layout.outerPetalCount) * BlossomConstants.petalAnimationDelay
