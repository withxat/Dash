...
public func extractHSB(from color: Color) -> (hue: Double, saturation: Double, brightness: Double) {
    ...
    return (
      nsColor.hueComponent * 360.0, nsColor.saturationComponent, nsColor.brightnessComponent * 100.0
    )
  #endif
}

/// Create a border color with adjusted saturation and brightness
public func adjustedBorderColor(
  from color: Color, saturationMultiplier: Double, brightnessMultiplier: Double
) -> Color {
  #if canImport(UIKit)
    let uiColor = UIColor(color)
    var h: CGFloat = 0
    ...
    return Color(
      NSColor(
        hue: nsColor.hueComponent, saturation: newS, brightness: newB, alpha: nsColor.alphaComponent
      ))
  #endif
}
...
public func colorsMatch(_ color1: Color, _ color2: Color, tolerance: Double = 0.01) -> Bool {
  ...
  let hue1 = h1 / 360.0
  let hue2 = h2 / 360.0
  let bright1 = b1 / 100.0
  let bright2 = b2 / 100.0
